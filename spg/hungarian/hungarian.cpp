...
void hungarian_print_matrix(double** C, int rows, int cols) {
  int i,j;
  fprintf(stderr , "\n");
  for(i=0; i<rows; i++) {
    fprintf(stderr, " [");
...
double hungarian_dmax(double a, double b) {
  return (a<b)?b:a;
}

double hungarian_init(hungarian_problem_t* p, double** cost_matrix, int rows, int cols, int mode) {
...
void hungarian_solve(hungarian_problem_t* p, double** assignment)
{
  ...
  slack_row  = (double*)calloc(p->num_rows,sizeof(double));
  ...
  for (i=0;i<m;++i)
    {
      assignment[i][col_mate[i]]=HUNGARIAN_ASSIGNED;
      /*TRACE("%d - %d\n", i, col_mate[i]);*/
    }
  for (k=0;k<m;++k)
    {
